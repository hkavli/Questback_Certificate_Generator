...
def send_email(navn, firmanavn, kursnavn, email, kursdato, kurssted, kursnavn2, kursdato2, pdf_filename, lenke, attestID, navnASCII):
    ...
    # OConnects to the server and sends email
    context = ssl.create_default_context()
    with smtplib.SMTP_SSL("smtp.gmail.com", PORT, context=context) as server:
        server.login(sender_email, password)
        if kurssted == "Oslo" or kurssted == "Trondheim":
            f1 = open("texts/textNO.txt", "r", encoding='utf-8')
            f2 = open("texts/htmlNO.txt", "r", encoding='utf-8')
            text = f1.read()
            html = f2.read()

        elif kurssted == "Stockholm":
            f1 = open("texts/textSV.txt", "r", encoding='utf-8')
            f2 = open("texts/htmlSV.txt", "r", encoding='utf-8')
            text = f1.read()
            html = f2.read()
            message["Subject"] = "Questback Essentials Certifikat"

        text2 = text.format(navn=navn, kursnavn=kursnavn, kursdato=kursdato,
                            kursnavn2=kursnavn2, kursdato2=kursdato2, attestID=attestID, lenke=lenke)
